...
import { Plus,Star,ArrowRight,ArrowDown,StarFilled } from '@element-plus/icons-vue/global';
...
onMounted(()=>{
  getList({classificationId:currentId.value}).then((res:anyObj)=>{
...
const itemsChecked = computed(()=>{
  return items.value?.filter(e=>e.isCompleted);
})
const itemsUnChecked = computed(()=>{
  return items.value?.filter(e=>!e.isCompleted);
})
      ...
      <el-main>
        ...
        <section class="contentList">
          <TransitionGroup tag="ul" name="fade">
            <div v-for="(item) in itemsUnChecked" :key="item.id" class="listItem">
              <div class="todoItem">
                <div>
          ...
          <el-button v-if="itemsChecked && itemsChecked.length > 0" :icon="buttonFlag?ArrowRight:ArrowDown" @click="buttonFlag = !buttonFlag" style="margin: 10px; 0">已完成</el-button>
          
          <TransitionGroup v-if="!buttonFlag" tag="ul" name="fade">
            <div v-for="item in itemsChecked" :key="item.id" class="listItem">
              <div class="todoItem">
                <div>
                  <el-checkbox v-model="item.isCompleted" class="custom-checkbox" @click="changeBox(item.id,item.isCompleted)"></el-checkbox>
                </div>
